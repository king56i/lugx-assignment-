<div class="container">
    <h2 class="py-2 text-center h2 mt-3">QUẢN LÝ HÃNG</h2>
    <table class="table table-hover table-bordered">
    <thead  style="background-color: #43dd8b;color:white" >
        <tr>
            <th>Tên Hãng</th>
            <th>Hình Ảnh</th>
            <th>Ẩn Hiện</th>
            <th>Mô Tả</th>
            <th>Người Tạo</th>
            <th colspan="2">
            <a class="btn btn-success" href="<?=URLROOT?>/admin/HangForm">Thêm Mới</a>
            </th>            
        </tr>
    </thead>
    <tbody>
        <?php
            foreach ($data['brandProducts'] as $item) {
                echo "<tr>
                        <td>$item[TenHang]</td>
                        <td><img src='".URLROOT."/images/$item[HinhAnh]' style='width:150px;margin:0 auto'/></td>
                        <td>$item[AnHien]</td>
                        <td>$item[MoTa]</td>
                        <td>$item[Uid]</td>
                        <td style=\"width:60px\"><a href='".URLROOT."/admin/HangForm/".$item['id_Hang']."'><button class=\"btn btn-success\">Sửa</button></a></td>
                        <td style=\"width:60px\"><button class=\"btn btn-success\" onclick='deleteHSP($item[id_Hang])'>Xóa</button></td>
                    </tr>";
            }
        ?>
    </tbody>
</table>
</div>
<script>
     deleteHSP=(id)=>{
         let check=confirm("Bạn có chắc chắn xóa không ??")
         console.log(id);
       if(check){
         $.post("<?=URLROOT?>/admin/xoaHSP", {'id_Hang':id}, (data)=>{
            if(data== 0) location.reload();          
            else console.log(data);    
         })
       }
     }
 </script>